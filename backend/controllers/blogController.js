...
const setBlogs = asyncHandler(async (req, res) => {
    ...
    if (!req.body.text) {
        res.status(400);
        throw new Error("Please add your blog'scontent...");
    };
...
const updateBlogs = asyncHandler(async (req,res) => {
    ...
    const updatedBlog = await Blog.findByIdAndUpdate(req.params.id, req.body.title || blog.title, req.body.text || blog.text, { new: true });
